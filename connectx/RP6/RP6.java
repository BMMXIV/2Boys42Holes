 ...
 * 
 */
public class RP6 implements CXPlayer {
	private Random rand;
	...
	private Score evaluateColumnsIterative(CXBoard B, Integer[] L, double alpha, double beta, int depth, boolean P) throws TimeoutException {
        ...
        save.column = L[rand.nextInt(L.length)];
		...
		while (true) {
            try {
                ...
                else {  //return random move if there wasn't enough time to evaluate not even with depth = 1
	...
	private double heuristicEval(CXBoard B, boolean P){
		...
		return (myPoints - yourPoints) * (M*N + 1 - B.numOfMarkedCells()) / (M*N + 1);
	}
	...
	private Integer[] checkVerticalLines(CXBoard B, boolean P){
		...
		for (int col = 0; col < N; col++) {
			...
			CXCellState actCell = B.cellState(M-1, col);
	...
	public String playerName() {
		return Integer.toString(prune) + " Visited Nodes: " + Integer.toString(nodes) +" Rdy Player 6";
	}
}
